...
import time
import threading
from browser_ai_gui.event_adapter import EventAdapter, LogEvent, EventType, LogLevel
...
def test_event_adapter():
    ...
    adapter.emit_custom_event(
        ...
        "Starting demo task",
        LogLevel.INFO,
        {"task": "Demo automation task"}
    )
    
    adapter.emit_custom_event(
        EventType.AGENT_ACTION, 
        "Clicking button with index 5",
        LogLevel.INFO
    )
    
    adapter.emit_custom_event(
        EventType.AGENT_RESULT,
        "Extracted content: Sample data from webpage",
        LogLevel.RESULT
    )
...
def simulate_browser_ai_session():
    ...
    def simulate_task():
        ...
        adapter.emit_custom_event(
            EventType.AGENT_ACTION,
            "⌨️ Input 'Python tutorials' into search box",
            LogLevel.INFO
        )
        ...
        adapter.emit_custom_event(
            EventType.AGENT_RESULT,
            "📄 Extracted top 5 search results",
            LogLevel.RESULT
        )
