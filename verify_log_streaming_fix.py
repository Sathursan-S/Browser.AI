...
try:
    from browser_ai_gui.protocol import LogEvent as ProtocolLogEvent, LogLevel, EventType
...
try:
    from browser_ai_gui.event_adapter import LogEvent as AdapterLogEvent, LogLevel as AdapterLogLevel, EventType as AdapterEventType
...
try:
    ...
    # Verify types
    assert isinstance(event.timestamp, str), f"timestamp should be str, got {type(event.timestamp)}"
    assert isinstance(event.level, str), f"level should be str, got {type(event.level)}"
    assert isinstance(event.event_type, str), f"event_type should be str, got {type(event.event_type)}"
...
try:
    ...
    assert event_dict['level'] == 'INFO', f"Expected 'INFO', got '{event_dict['level']}'"
    assert event_dict['event_type'] == 'agent_start', f"Expected 'agent_start', got '{event_dict['event_type']}'"
    print(f"✅ Enum values correctly converted: level={event_dict['level']}, event_type={event_dict['event_type']}")
