...
class ConfigDialog:
	...
	def setup_ui(self):
		...
		temp_scale = ttk.Scale(llm_frame, from_=0.0, to=2.0, variable=self.temperature_var, orient=HORIZONTAL, length=200)
	...
	def save_config(self):
		...
		try:
			...
			self.config_manager.update_browser_config(
				headless=self.headless_var.get(), disable_security=self.disable_security_var.get()
			)

			self.config_manager.update_agent_config(
				use_vision=self.use_vision_var.get(), max_steps=self.max_steps_var.get(), max_failures=self.max_failures_var.get()
			)

			# Validate configuration
			issues = self.config_manager.validate_config()
			if issues:
				messagebox.showwarning('Configuration Issues', 'Configuration saved but has issues:\n' + '\n'.join(issues))
			else:
...
class BrowserAIGUI:
	...
	def setup_styles(self):
		...
		style.configure('Header.TLabel', background='#2d2d30', foreground='#cccccc', font=('Segoe UI', 12, 'bold'))
	...
	def setup_sidebar(self, parent):
		...
		ttk.Label(header_frame, text='Browser.AI Assistant', style='Header.TLabel').pack(anchor=W)

		# Status indicator
		status_frame = ttk.Frame(self.sidebar_frame, style='Sidebar.TFrame')
		status_frame.pack(fill=X, padx=10, pady=(0, 10))

		self.status_label = ttk.Label(status_frame, text='● Stopped', style='Status.TLabel', foreground='#dc3545')
		...
		self.pause_btn = ttk.Button(button_frame, text='Pause', command=self.pause_task, state=DISABLED)
		self.pause_btn.pack(side=LEFT, padx=(0, 5))

		self.stop_btn = ttk.Button(button_frame, text='Stop', command=self.stop_task, state=DISABLED)
		self.stop_btn.pack(side=LEFT, padx=5)

		# Configuration button
		ttk.Button(button_frame, text='Config', command=self.open_config).pack(side=RIGHT)

		# Separator
		ttk.Separator(self.sidebar_frame, orient=HORIZONTAL).pack(fill=X, pady=10)

		# Current task info
		ttk.Label(self.sidebar_frame, text='Current Task:', style='Status.TLabel').pack(anchor=W, padx=10, pady=(0, 5))

		self.task_label = ttk.Label(self.sidebar_frame, text='None', style='Status.TLabel', wraplength=280)
	...
	def setup_chat_area(self, parent):
		...
		ttk.Label(chat_header, text='Browser Automation Chat', style='Header.TLabel').pack(anchor=W)

		# Chat messages area
		messages_frame = ttk.Frame(self.chat_frame)
		messages_frame.pack(fill=BOTH, expand=True, padx=20, pady=10)

		self.chat_text = ScrollableText(
			messages_frame, font=('Segoe UI', 10), bg='#1e1e1e', fg='#cccccc', selectbackground='#264f78'
		)
		...
		self.add_chat_message(
			"Hello! I'm your Browser.AI assistant. I can help you automate web tasks.\n\n"
			'To get started:\n'
		...
		self.input_text = Text(
			input_container, height=3, wrap=WORD, font=('Segoe UI', 10), bg='#3c3c3c', fg='#cccccc', insertbackground='#cccccc'
		)
		...
		# Send button
		self.send_btn = ttk.Button(input_frame, text='Send Task', command=self.send_message)
