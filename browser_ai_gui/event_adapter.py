...
class LogCapture(logging.Handler):
    ...
    def _determine_event_type(self, record: logging.LogRecord) -> EventType:
        ...
        if "starting task" in message:
            ...
            action in message
            for action in ["clicked", "navigated", "input", "scrolled"]
        ):
            ...
            "requesting user help" in message
            or "🙋‍♂️" in record.getMessage()
            or "task requires user intervention" in message
        ):
        ...
        while self._running:
            try:
                ...
                for callback in self.subscribers[
                    :
                ]:  # Copy list to avoid issues during iteration
                    try:
                        callback(event)
                    except Exception:
