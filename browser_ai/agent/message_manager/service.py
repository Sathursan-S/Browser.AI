...
from browser_ai.agent.views import ActionResult, AgentOutput, AgentStepInfo
...
class MessageManager:
    ...
    def _add_message_with_tokens(
        ...
        # Null safety check
        if message is None:
            logger.warning("Attempted to add None message to history - skipping")
            return
        ...
        # Null safety check after filtering
        if message is None:
            logger.warning(
                "Message became None after sensitive data filtering - skipping"
            )
            return
    ...
    def _count_tokens(self, message: BaseMessage) -> int:
        ...
        if message is None or message.content is None:
            logger.warning(
                "Attempted to count tokens for None message or None content - returning 0"
            )
            return 0
    ...
    def extract_json_from_model_output(self, content: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                # If JSON parsing fails, try to convert YAML-like format to JSON
                # This handles Gemini's conversational output format
                logger.debug(
                    "JSON parsing failed, attempting to convert from conversational format"
                )
                return self._convert_conversational_to_json(content)

        except json.JSONDecodeError as e:
            logger.warning(f"Failed to parse model output: {content} {str(e)}")
            raise ValueError("Could not parse response.")

    def _convert_conversational_to_json(self, content: str) -> dict:
        """Convert Gemini's conversational YAML-like format to JSON.

        Example input:
        Current state:
        - page_summary: The user is on...
        ...
        result = {"current_state": {}, "action": []}

        # Extract current_state section
        current_state_match = re.search(
            r"Current state:\s*\n(.*?)(?=Action:|$)", content, re.DOTALL | re.IGNORECASE
        )

        if current_state_match:
            state_content = current_state_match.group(1)
            # Parse each field in current_state
            for field in [
                "page_summary",
                ...
                field_match = re.search(
                    pattern, state_content, re.DOTALL | re.IGNORECASE
                )
                if field_match:
                    result["current_state"][field] = field_match.group(1).strip()
        ...
        if action_match:
            ...
            # Parse each action (format: - action_name: {params})
            action_pattern = r"-\s*(\w+):\s*(\{[^}]+\})"
            for match in re.finditer(action_pattern, action_content):
                action_name = match.group(1)
                action_params = json.loads(match.group(2))
                result["action"].append({action_name: action_params})

        if not result["current_state"] or not result["action"]:
            raise ValueError(
                "Could not extract required fields from conversational format"
            )

        logger.debug(f"Converted conversational format to JSON: {result}")
        return result
