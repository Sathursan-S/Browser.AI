...
class SearchEcommerceAction(BaseModel):
    query: str
    site: Optional[str] = (
        None  # e.g., 'daraz.lk', 'ikman.lk', 'glomark.lk', 'amazon.com', 'ebay.com', etc.
    )
...
class FindBestWebsiteAction(BaseModel):
    ...
    category: (
        str  # Category: "shopping", "download", "information", "service", or "other"
    )
...
class AskUserQuestionAction(BaseModel):
    ...
    options: list[str] = (
        []
    )  # Optional: Suggested answers/options for the user to choose from
