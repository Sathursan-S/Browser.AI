...
from browser_ai.browser.views import (
    BrowserError,
    BrowserState,
    TabInfo,
    URLNotAllowedError,
...
from browser_ai.dom.views import DOMElementNode, SelectorMap
from browser_ai.utils import time_execution_sync
...
class BrowserContext:
    ...
    @observe(name="browser.get_state")
    @time_execution_sync(
        "--get_state"
    ...
    async def get_state(self) -> BrowserState:
        ...
        with Laminar.start_as_current_span(
            ...
            Laminar.set_span_output(
                {
                    "url": session.cached_state.url,
                    "title": session.cached_state.title,
                    "element_count": len(session.cached_state.selector_map),
                    "tabs": len(session.cached_state.tabs),
                }
            )
